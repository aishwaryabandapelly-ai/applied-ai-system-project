...
The original system retrieved only from `data/songs.csv`. This stretch feature adds a small, local, multi-source retrieval step that consults two hand-authored knowledge documents before recommendations are generated — with no external API, LLM, vector database, or internet service.

The observable retrieval flow is:

1. The agent reads the requested genre and optional listening context.
2. `retrieve_knowledge(...)` looks up the genre in [knowledge/genre_aliases.json](knowledge/genre_aliases.json) and the context in [knowledge/listening_contexts.md](knowledge/listening_contexts.md).
3. A genre alias is normalized to its canonical catalog genre (e.g. `edm` → `electronic`), and matched context attributes are used to fill **only missing** profile fields.
4. Explicit user preferences are never overwritten; conflicts are recorded as warnings, and unsupported aliases/contexts fall back safely with no invented data.
5. The enriched profile flows through the normal pipeline, and the retrieval is recorded as an observable `knowledge_retrieval` step in the execution trace plus a `retrieved_knowledge` field on the result.

A verified before/after comparison (baseline vs knowledge-enhanced, using the integrated agent) is documented in [experiments/rag_comparison_results.md](experiments/rag_comparison_results.md), generated by [experiments/run_rag_comparison.py](experiments/run_rag_comparison.py).

---

## Stretch Feature: Fine-Tuning / Specialization

**Why deterministic specialization was chosen.** The rubric permits specializing behavior without training a neural network. Given the constraints (no external LLM/API, no embeddings, no new dependencies, fully deterministic), I implemented a natural-language preference interpreter as documented keyword rules rather than a learned model. This keeps the system reproducible, inspectable, and testable — every inference can be traced to a specific rule.

**How few-shot examples guided implementation.** I first wrote a small synthetic dataset of ~24 example requests paired with the profile each should produce ([specialization/preference_examples.json](specialization/preference_examples.json)). Those examples acted as the "few-shot" specification: the interpreter's activity and attribute rules were written to reproduce the dataset's mappings (e.g. *studying → lofi / focused / low energy / acoustic*), and each interpretation reports which dataset examples share its matched keywords.

**How it differs from true fine-tuning.** True fine-tuning adjusts a model's weights from data; here nothing is trained and no weights exist. The "specialization" is a deterministic mapping layer: it fills only missing profile fields (explicit user values win), assigns a transparent High/Medium/Low confidence, and records an observable `preference_interpretation` trace step. A verified before/after comparison is in [experiments/specialization_results.md](experiments/specialization_results.md), generated by [experiments/run_specialization_comparison.py](experiments/run_specialization_comparison.py).
